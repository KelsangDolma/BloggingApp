...
import com.takeo.exception.PasswordMismatchException;
import com.takeo.exception.ResourceNotFoundException;
import com.takeo.exception.DuplicateItemException;
import com.takeo.exception.InvalidImageFileExtension;
...
//		
	...
	public String register(UserDto userDto) {
		...
		if (existingUser.isPresent()) {
			...
			try {
				// Send OTP via email
				emailSender.sendMail(userDto.getEmail(), "OTP", "Your OTP is " + otp);
	...
	public String verifyOtp(String otp) {
		...
		if (otp.equals(existingUser.getOtp())) {
			...
			User saveUser = daoImpl.save(existingUser);
			if (saveUser != null) {
				emailSender.sendMail(existingUser.getEmail(), "Password", "Your password is " + randomPassword);
	...
	public String forgotPassword(String email) {
		...
		User saveUser = daoImpl.save(user);
		if (saveUser != null) {
			emailSender.sendMail(user.getEmail(), "Password", "Your password is " + randomPassword);
	...
	public String updateProfilePicture(MultipartFile file, String email) {
		...
		throw new InvalidImageFileExtension("Only image files are allowed.");
	}
